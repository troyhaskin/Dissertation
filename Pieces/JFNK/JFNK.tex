...
As such, a version called Simpler GMRES was later developed by Walker and Zhou to build an orthonormal basis for $A\mathcal{K}_m(A,r_0)$ and use those basis vectors to form $Z_m$ \cite{walker_simpler_1994}.
The choice of vectors in Simpler GMRES leads directly to a QR factorization and is therefore simpler to implement; however, the choice also leads to a method that can result in an ill-conditioned $R$-factor and therefore be far less acurate than the classic method \cite{jiranek_how_2008}.
...
To wit, at the $m$-th iteration of the solution, given a Simpler GMRES basis vector $q_{m-1}$ and residuals from two previous iterations ${r_{m-1}\,,\,r_{m-2}}$, the vector $z_m$ is assigned thusly \cite{jiranek_adaptive_2009}:
\begin{equation}
    z_m =
    \begin{cases}
